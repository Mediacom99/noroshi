...
- Any HTTP 2xx status counts as UP unless the endpoint has an exact expected status (`/expect`) or required keyword (`/keyword`); connection errors, wrong status, and missing keyword are all DOWN. The failure reason is persisted in `last_check_error`.
